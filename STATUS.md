...
- **Guida immagini AI** (Google Nano Banana / Gemini 2.5 Flash Image): stile visivo fisso della testata (illustrazione fumetto europeo + acquerello, palette pastello, 16:9) documentato nella skill `.claude/skills/article-images/SKILL.md` e nel Google Doc "Bike-Style-Mag_Guida-Immagini" in `00_GUIDE BIKE STYLE` su Drive. Al momento la generazione dell'immagine va comunque fatta manualmente (nessuna integrazione API automatica ancora attiva) — vedi voce sotto.
...
- **Automazione generazione immagini**: valutare con l'utente come collegare la creazione automatica dell'immagine di copertina (in base al titolo della news) al flusso di scrittura bozze — serve una decisione su dove/come chiamare l'API di generazione immagini (chiave Gemini, dove custodirla, trigger: manuale, GitHub Action, o nello scraper `bikestylemag-idea`). Nessun tool di generazione immagini è disponibile direttamente in questa sessione.
